...
# Subversion changes for 0.99f:
#    - Dramatically reduced memory footprint by using tree iterator instead of tree list
#
...
def pp_calc(args, i):
    ...
    # Get burnin from MrConverge
    if not args.manual:
        args.burnin = getMrCburn(args, i)
        
    tree_iter = get_tree_iter(args, i)
    ...
    if args.quantile or args.entropy or args.iqr or args.length or args.tlVar or len(args.part_list) > 0:
        ...
        unique_topologies = dendropy.TreeList()
        topology_counts = []
        unique_topo_dists = {}
        tree_lengths = []
        first_tree = True
        ...
        while True: # Get trees one at a time until there aren't any more
            try:
                tree = tree_iter.next()
            except StopIteration:
                break
                
            if first_tree: # Get number of taxa from first tree
                first_tree = False
                ntaxa = len(tree.taxon_set)
                
            
            # Update list of unique topologies, topology counts, and topology distances
            match_topo = find_matching_topology(args, unique_topologies, tree, i)
            if len(match_topo) == 0 or match_topo[-1] == 0: # If tree is unique
                topology_counts.append(1)
                unique_topologies.append(tree)
                unique_topo_dists[len(topology_counts)-1] = []
                #Calculate distances between unique topologies and current tree
                for num, unique_topo in enumerate(match_topo):
                    dist = match_topo[num]
                    unique_topo_dists[num].append(dist)
                
            else:
                topology_counts[len(match_topo) - 1] += 1
                
            tree_lengths.append(tree.length())
            
            if len(args.part_list) < 0:
                treelist = dendropy.TreeList(tree)
                split_count += treelist.frequency_of_split(args.part_list)          
    
    rf_dists_tally = tally_rf_dists(topology_counts, unique_topo_dists)
    
    # Calculate tree length and variance test statistics
    if args.length:
        pp_tls = sum(tree_lengths)/float(len(tree_lengths))
    else:
        pp_tls = []
        
    if args.tlVar:
        pp_tlVar = variance(tree_lengths)
    else:
        pp_tlVar = []
    ...
    if args.quantile:
        pp_quantiles = quantile_test(args, rf_dists_tally)    
    else:
        pp_quantiles = []    

    # Calculate entropy-based test statistic
    if args.entropy:
        pp_entropies = entropy_test(args, topology_counts, unique_topo_dists, ntaxa)
    ...
    if args.iqr:
        pp_iqr = iqr_test(args, rf_dists_tally)
    else:
        pp_iqr = []
...
def find_matching_topology(args, unique_topologies, tree, i):
    """
    A function to take a tree and return a matching topology. If no match is found, return -1.
    """
    
    dists = []
    try:
        if len(unique_topologies) == 0:
            return dists
        else:
            for num, unique_topo in enumerate(unique_topologies):
                dists.append(num)
                if tree.symmetric_difference(unique_topo) == 0:
                    return dists
            return dists
            
    except:
        print '\nProblem identifying if tree has match in rep {rep}...'.format(rep = i)
        if args.debug:
            traceback.print_exc()
        sys.exit(1)
        
def tally_rf_dists(topology_counts, unique_topo_dists):
    """
    Create distance:count tally for a single replicate dataset.
    """
    ...
    # Get max rf_dist
    max_rf_dist = 0
    for key, value in sorted(unique_topo_dists.items()):
        for dist in value:
            if dist > max_rf_dist:
                max_rf_dist = dist
    
    # Initialize rf_dists_tally based on max rf distance
    for dist in range(0, max_rf_dist + 1, 2):
        rf_dists_tally[dist] = 0
    
    for num_a, count in enumerate(topology_counts):
        for num_b, dist in enumerate(unique_topo_dists[num_a]):
            rf_dists_tally[dist] += topology_counts[num_a + num_b]
            
    return rf_dists_tally
        
def quantile_test(args, rf_dists_tally):
    """
    Calculates positions of a set of k-th p-quantiles from tally of RF distances 
    ...
    try:
        ...
        num_trees = sum(rf_dists_tally)
        quantile = []
        for i in range(len(k)):
            # Iterate and find k-th p-quantiles
            #     See definition of k-th p-quantile position in my manuscript
            g = (num_trees * int(k[i])) % int(p[i])
            j = (num_trees * int(k[i])) / int(p[i])
            if g == 0:
                quantile.append((float(get_dist_from_tally(args, rf_dists_tally, j))+float(get_dist_from_tally(args, rf_dists_tally, j+1)))/2)
            else:
                quantile.append(float(get_dist_from_tally(args, rf_dists_tally, j+1)))
        return quantile
...
def get_dist_from_tally(args, rf_dists_tally, index):
    """
    Returns RF distance from tally based on index.
    """
    if index < 0:
        raise Exception('Index out of bounds for quantile test statistic.')
    for dist, count in sorted(rf_dists_tally.items()):
        index -= count
        if index <= 0:
            return dist
    raise Exception('Index out of bounds for quantile test statistic.')

def entropy_test(args, topology_counts, unique_topo_dists, ntaxa):
    """
    ...
    try:
        total_trees = sum(topology_counts)
        ...
        entropy = 0
        for j in range(len(topology_counts)):
            entropy += topology_counts[j] * math.log(topology_counts[j])
...
def iqr_test(args, rf_dists_tally):
    ...
    try:
        ...
        j = total_dists / 4
        if g == 0:
            first_quant = float(get_dist_from_tally(args, rf_dists_tally, j) + get_dist_from_tally(args, rf_dists_tally, j+1)) / 2
        else:
            first_quant = float(get_dist_from_tally(args, rf_dists_tally, j + 1))
        ...
        j = (total_dists * 3) / 4
        if g == 0:
            third_quant = float(get_dist_from_tally(args, rf_dists_tally, j) + get_dist_from_tally(args, rf_dists_tally, j+1)) / 2
        else:
            third_quant = float(get_dist_from_tally(args, rf_dists_tally, j + 1))  
...
def partition_test(args, topology_counts, split_count):
    """
    ...
    try:
        total_trees = sum(topology_counts)
        p = float(split_count) / total_trees
        if p < 1 and p > 0:
